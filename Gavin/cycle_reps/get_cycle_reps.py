...
def main():
    ...
    with ProcessPool(max_workers=NUM_PROCESSES) as pool: # run NUM_PROCESSES workers at once
        ...
        # for id in homology[DIM_CONDITION(homology['dimension'])].index: # calculate it for every cycle
        #     future = pool.schedule(optimize_cycle, (homology.loc[id], concepts), timeout=TIMEOUT_LEN) # start the process
        #     id_callback = partial(callback, id, concepts, results, errors)
        #     future.add_done_callback(id_callback)
    it = futures.result()
